...
const Menu = () => {
  ...
  const links = [
    ...
    //  icon: <PiWarningCircleBold className="text-[20px] text-[red]" />
     },
    ...
    <div className="w-[100%] sm:text-[14px] text-[11px] text-[#fff] font-[Poppins] block items-center justify-center h-[60px] md:hidden">
      {links.map((link) => (
        <Link
          key={link.to}
          to={link.to}
          className={`h-[auto] flex items-center mt-4 ${location.pathname === link.to ? "text-[#318AE6]" : ""}`}
        >
          {link.icon}
      ...
      <span onClick={handleLogout} className="flex items-center mt-5"> Log Out</span>
    </div>
  );
};
